...
# ============================================================
# Resolve Model (verbatim from run_pipeline.sh)
# ============================================================
...
if _hk and _hb and "bedrock" in model_name:
    for _suf, _short in _HELICONE_MODEL_MAP.items():
        if _suf in model_name:
            model_name = _short
            print(f"PROBE_INFO: Helicone remap -> {_short}")
            break
    for _k in ("AWS_BEARER_TOKEN_BEDROCK", "AWS_ACCESS_KEY_ID",
                "AWS_SECRET_ACCESS_KEY", "AWS_SESSION_TOKEN", "AWS_PROFILE"):
        os.environ.pop(_k, None)
    _probe_kw["api_base"] = _hb
    _probe_kw["api_key"] = _hk
    _probe_kw["aws_region_name"] = "ap-south-1"
